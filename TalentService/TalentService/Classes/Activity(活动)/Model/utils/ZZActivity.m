...
-(void)setStartTime:(NSString *)startTime{
  _startTime =  [self  dealHourDate:startTime];
   
}

-(void)setEndTime:(NSString *)endTime{
    _endTime =  [self  dealHourDate:endTime];
}

- (void)setApplyStartTime:(NSString *)applyStartTime{
    _applyStartTime = [self  dealNoHourDate:applyStartTime];
}

-(void)setApplyEndTime:(NSString *)applyEndTime{
    _applyEndTime = [self dealNoHourDate:applyEndTime];
}
...
-(NSString *)detailRule{
    if (_detailRule == nil) {
        ...
        if (self.address.length) {
            [mdetailRule  appendFormat:@"服务地点:%@\n",self.address];
        }
        if (self.peoples) {
            [mdetailRule  appendFormat:@"服务参加人数:%ld人",self.peoples];
        }
...
/**日期转换*/
- (NSString *)dealNoHourDate:(NSString *)originDate{
